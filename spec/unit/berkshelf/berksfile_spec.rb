...
EOF
    ...
    describe "#sources" do
      ...
      it 'returns an array of Berkshelf::CookbookSource' do
        expect(subject.sources).to be_a Array
        expect(subject.sources.first).to be_a Berkshelf::CookbookSource
      end
    end

    describe "#filter" do
      before do
        Berkshelf::Berksfile.send(:public, :filter)
      ...
      let(:groups) { [:nautilus, :skarner] }

      context "given the option :except" do
        it "returns all of the sources except the ones in the given groups" do
          filtered = subject.filter(sources, except: :nautilus)

          expect(filtered).to have(1).item
          expect(filtered.first).to be(source_one)
        end
      end

      context "given the option :only" do
        it "returns only the sources in the given groups" do
          filtered = subject.filter(sources, only: :nautilus)

          expect(filtered).to have(1).item
          expect(filtered.first).to be(source_two)
        end
      end

      context "given the option :cookbooks" do
        it "returns only the sources whose names are in the array" do
          filtered = subject.filter(sources, cookbooks: ['nginx'])
      ...
      context "when a value for :only and :except is given" do
        it "raises an ArgumentError" do
          expect {
            subject.filter(sources, only: [:default], except: [:other])
    ...
    describe "#install" do
      ...
      context "when a value for :except is given" do
        ...
        it "filters the sources and gives the results to the Resolver initializer" do
          subject.should_receive(:filter).with(sources, except: [:skip_me]).and_return(sources)
      ...
      context "when a value for :only is given" do
        ...
        it "filters the sources and gives the results to the Resolver initializer" do
          subject.should_receive(:filter).with(sources, only: [:skip_me]).and_return(sources)
