Feature: install cookbooks from a Berksfile
  ...
  Scenario: installing a Berksfile when a lockfile is present
    Given I write to "Berksfile" with:
      """
      cookbook "ntp"
    ...
    Then the output should contain:
      """
      Installing ntp (1.1.0) from site: 'http://cookbooks.opscode.com/api/v1/cookbooks'
      Installing build-essential (1.1.0) from site: 'http://cookbooks.opscode.com/api/v1/cookbooks'
      """

  Scenario: installing when the Berksfile has a mismatched version with the lockfile
    Given I write to "Berksfile" with:
      """
      cookbook "ntp", "~> 2.0.0"
      """
    ...
    Then the output should contain:
      """
      The current lockfile has ntp locked at 1.1.0.
      Try running `berks update ntp`
      """
