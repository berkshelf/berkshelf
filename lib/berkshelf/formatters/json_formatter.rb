module Berkshelf
  class Formatters::JSONFormatter < Formatter
    ...
    def install(cookbook, version, options = {})
      ...
      if options.has_key?(:api_source) && options.has_key?(:location_path)
        cookbooks[cookbook][:api_source] = options[:api_source] unless options[:api_source] == Berksfile::DEFAULT_API_URL
        cookbooks[cookbook][:location_path] = options[:location_path] unless options[:api_source] == Berksfile::DEFAULT_API_URL
      end
    end
